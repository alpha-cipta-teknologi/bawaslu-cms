...
// ** Store & Actions
import { getBawasluUpdate, deleteBawasluUpdate } from '../store/action'
...
const handleDelete = (row) => {
  return MySwal.fire({
    ...
    if (result.value) {
      store.dispatch(deleteBawasluUpdate(row.id))
    }
  })
}
    ...
    },
    {
      ...
      cell: row => (
        <UncontrolledDropdown>
          ...
          <DropdownMenu right>
            {ability.can('edit', 'bawaslu_update') &&
              <DropdownItem
                tag={Link}
                to={`/bawaslu_update/edit/${row.id}`}
                className='w-100'
                onClick={() => store.dispatch(getBawasluUpdate(row))}
              >
                <Archive size={14} className='mr-50' />
                <span className='align-middle'>Edit</span>
              </DropdownItem>
            }
            {ability.can('delete', 'bawaslu_update') &&
