...
// ** Store & Actions
import { addHasilCekFakta, updateHasilCekFakta } from '../store/action'
...
const HasilCekFaktaSave = () => {
  // ** States & Vars
  const store = useSelector(state => state.hasilcekfaktas),
  ...
  useEffect(() => {
    ...
    if (store.success) {
      ...
      history.push("/hasil_cek_fakta/list")
    } else if (store.error) {
      toast.error(
  ...
  const onSubmit = data => {
    ...
    if (isObjEmpty(errors)) {
      ...
      datas.append('judul', data.judul)
      datas.append('link_berita', data.link_berita)
      datas.append('image', logo.file)

      if (id) {
        dispatch(updateHasilCekFakta(id, datas))
      } else {
        dispatch(addHasilCekFakta(datas))
      }
              ...
              <Row className='mt-1'>
                <Col sm='12'>
                  <h4 className='mb-1'>
                    ...
                    <span className='align-middle'>{`${isEdit ? 'Edit' : 'Tambah'} Hasil Cek Fakta`}</span>
                  </h4>
                </Col>
                {store.progress &&
                  <Col sm='12'>
                ...
                <Col sm='12'>
                  <FormGroup>
                    ...
                    <Input
                      id='judul'
                      name='judul'
                      defaultValue={isEdit ? store.selected.judul : ''}
                      placeholder='Judul'
                ...
                <Col sm='12'>
                  <FormGroup>
                    ...
                    <Input
                      id='link_berita'
                      name='link_berita'
                      defaultValue={isEdit ? store.selected.link_berita : ''}
                      placeholder='Link Berita'
                      innerRef={register({ required: true })}
                      className={classnames({
                        'is-invalid': errors.link_berita
                      })}
                    />
                  </FormGroup>
                </Col>
...
export default HasilCekFaktaSave
