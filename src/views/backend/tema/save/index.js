...
// ** Store & Actions
import { addTema, updateTema } from '../store/action'
...
const TemaSave = () => {
  // ** States & Vars
  const store = useSelector(state => state.temas),
  ...
  const onSubmit = data => {
    ...
    if (isObjEmpty(errors)) {
      ...
      datas.append('tema_name', data.tema_name)
      datas.append('show_order', data.show_order)
      datas.append('icon_image', logo.file)

      if (id) {
        dispatch(updateTema(id, datas))
      } else {
        dispatch(addTema(datas))
      }
              ...
              <Row className='mt-1'>
                <Col sm='12'>
                  <h4 className='mb-1'>
                    ...
                    <span className='align-middle'>{`${isEdit ? 'Edit' : 'Tambah'} Tema`}</span>
                  </h4>
                </Col>
                {store.progress &&
                  <Col sm='12'>
                ...
                <Col sm='12' md='8'>
                  <FormGroup>
                    <Label for='tema_name'>Nama Tema</Label>
                    <Input
                      id='tema_name'
                      name='tema_name'
                      defaultValue={isEdit ? store.selected.tema_name : ''}
                      placeholder='Nama Tema'
                      innerRef={register({ required: true })}
                      className={classnames({
                        'is-invalid': errors.tema_name
                      })}
                    />
                  </FormGroup>
                </Col>
